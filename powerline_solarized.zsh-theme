...
# else
    if [ "$POWERLINE_SHOW_GIT_ON_RIGHT" = "" ]; then
        if [ "$POWERLINE_HIDE_GIT_PROMPT_STATUS" = "" ]; then
            POWERLINE_GIT_INFO_LEFT=" %F{033}%K{255}""%F{255}%F{black}%K{255}"$'$(git_prompt_info)$(git_prompt_status)%F{255}'
        else
            POWERLINE_GIT_INFO_LEFT=" %F{033}%K{255}""%F{255}%F{black}%K{255}"$'$(git_prompt_info)%F{255}'
        fi
        POWERLINE_GIT_INFO_RIGHT=""
    else
        POWERLINE_GIT_INFO_LEFT=""
...
if [ $(id -u) -eq 0 ]; then
    ...
    POWERLINE_SEC1_BG=%K{000}
    POWERLINE_SEC1_FG=%F{007}
	POWERLINE_SEC1_TXT=%F{255}
fi
...
PROMPT="$POWERLINE_SEC1_BG $POWERLINE_SEC1_TXT $POWERLINE_USER_NAME %k%f$POWERLINE_SEC1_FG%K{012}""%k%f%F{black}%K{012} "$POWERLINE_CURRENT_PATH"%F{012}"$POWERLINE_GIT_INFO_LEFT" %k""%f "
...
if [ "$POWERLINE_DISABLE_RPROMPT" = "" ]; then
    if [ "$POWERLINE_RIGHT_A" = "" ]; then
        RPROMPT="$POWERLINE_GIT_INFO_RIGHT%F{255}""%k%F{black}%K{255} $POWERLINE_RIGHT_B %f%k"
    elif [ "$POWERLINE_RIGHT_B" = "" ]; then
        RPROMPT="$POWERLINE_GIT_INFO_RIGHT%F{255}""%k%F{240}%K{255} $POWERLINE_RIGHT_A %f%k"
    else
        RPROMPT="$POWERLINE_GIT_INFO_RIGHT%F{255}""%k%F{black}%K{255} $POWERLINE_RIGHT_B %f%F{240}""%f%k%K{240}%F{255} $POWERLINE_RIGHT_A %f%k"
    fi
fi
