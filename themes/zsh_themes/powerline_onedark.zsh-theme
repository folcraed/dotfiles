...
# else
    if [ "$POWERLINE_SHOW_GIT_ON_RIGHT" = "" ]; then
        if [ "$POWERLINE_HIDE_GIT_PROMPT_STATUS" = "" ]; then
            POWERLINE_GIT_INFO_LEFT=" %F{012}%K{255}""%F{255}%F{black}%K{255}"$'$(git_prompt_info)$(git_prompt_status)%F{255}'
        else
            POWERLINE_GIT_INFO_LEFT=" %F{012}%K{255}""%F{255}%F{black}%K{255}"$'$(git_prompt_info)%F{255}'
        fi
        POWERLINE_GIT_INFO_RIGHT=""
    else
        POWERLINE_GIT_INFO_LEFT=""
